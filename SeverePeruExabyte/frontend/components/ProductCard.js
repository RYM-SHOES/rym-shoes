    ...
    <div className="group relative">
      <Link href={`/product/${product.id}`}>
        <div className="relative overflow-hidden aspect-[3/4] bg-beige mb-4">
          ...
          <button
            onClick={handleAdd}
            className={`absolute bottom-4 left-4 right-4 py-2.5 text-[10px] font-semibold tracking-widest uppercase
              translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100
              transition-all duration-300
              ${added ? 'bg-dark-green text-cream' : 'bg-cream text-warm-black hover:bg-dark-green hover:text-cream'}`}
          >
